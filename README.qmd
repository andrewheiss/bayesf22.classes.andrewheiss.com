...
[Andrew Heiss](https://www.andrewheiss.com/) and [Meng Ye](https://aysps.gsu.edu/phd-student/ye-meng/) • Andrew Young School of Policy Studies • Georgia State University
...
- **Run long-running Bayesian scripts**: Bayesian computation with MCMC sampling takes a long time. Instead of running our models in `.qmd` files that are re-run when rendering the website, we run them in separate `.R` files and show their results in their corresponding `.qmd` files.
