...
- **Run long-running Bayesian scripts**: Bayesian computation with MCMC sampling takes a long time. Instead of running our models in `.qmd` files that are re-run when rendering the website, we run them in separate `.R` (located in [`R/`](R/)) files and show their results in their corresponding `.qmd` files.
